...
void setup() 
{
  ...
  //wait for capacitive sensor to calibrate
...
void loop() 
{
  ...
  if (dis < 80)
  {
    ...
    if (dis < 80)  //check
    {
      ...
      {
        dis = SharpIR.distance();  // this returns the distance to the object you're measuring
        if (dis < 80)  //person there
        {
          Serial.println("Person still there");
          timeout = 0;
          delay(200);  //wait
          
          //Or we can handle Manual Flush
        ...
        else
        {
          ...
          timeout ++;
          delay(200);  //wait 10 of these for 2 second delay
        }
      }
